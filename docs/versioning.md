...
## versionName = pubspec의 x.y.z — 시맨틱 버저닝, 배포마다 올린다

`pubspec.yaml`의 `version: 1.1.0+1`에서 **앞부분만** 씁니다. `+1`은
...
| 자리 | 언제 올리나 | 예 |
|---|---|---|
| MAJOR | 대규모 개편, 데이터/호환성이 갈리는 변화 | 전면 리디자인, 마이그레이션 필수 |
| MINOR | **기능이 포함된 배포 묶음** (`feat` 커밋이 하나라도 있으면) | 함께 운동 추가 |
| PATCH | 버그 수정만 있는 배포 묶음 | 버튼 색 수정 |

**모든 배포는 이전 배포와 versionName이 달라야 합니다.** "의미 있으면 올리세요"
라는 예전 규칙은 기준이 없어서 1.0.0이 백 빌드 넘게 그대로였습니다 — 이제
배포 스크립트와 CI가 마지막 `dist/*` 태그 시점의 pubspec과 비교해 같으면
**빌드 전에 중단**시킵니다. 문서가 아니라 도구가 규칙을 지킵니다.

스토어 정식 출시는 그 시점의 현재 버전으로 나갑니다 — 출시를 위해 번호를
되돌리거나 맞추지 않습니다.
