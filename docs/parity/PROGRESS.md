...
- 운영 데이터: 역할별 KPI·성과·상태·업무·알림을 Repository 스냅샷 스키마 v3로 전환하고 v1/v2 하위 호환 유지.
- 운영 상태: 새로고침 스켈레톤, Repository 기반 빈 상태, 성공 피드백, 저장소 오류 배너와 재시도 동작 구현.
- 알림: 역할 홈에서 확인 가능한 알림 바텀시트, 개별 스와이프 처리, 모두 읽음, 빈 상태와 읽음 상태 영속 구현.
...
- 디자인 시스템: 화면 패딩 24dp, 카드 라운드 20dp, BottomNav 64dp, 토큰 타이포·역할별 semantic 색상 적용.
- 다크모드: 사업자 설정에 테마 토글을 추가하고 설정·역할 홈·카드·내비게이션의 동등한 다크 화면과 영속성 구현.
...
- `flutter analyze`: No issues.
- `flutter test`: 29개 통과.
- 신규 `business_home_test.dart`: 역할별 셸, 스켈레톤 새로고침, 알림 영속·빈 상태, 상담 진입, 역할별 운영 정보, 저장 실패 재시도, 다크모드와 설정 영속 검증.
- `app_repository_test.dart`: 스키마 v3 사업자 데이터 왕복과 스키마 v1/v2 하위 호환 검증.
- `flutter build web`: 성공, Wasm dry run 성공.
- 432×900 모바일 뷰포트에서 트레이너 등록부터 홈 진입, 알림 읽음 후 새로고침 영속, 라이트·다크 전환 및 브라우저 오류 없음 확인.
...
# Phase D-5 — 사업자 상세 화면군 프로덕션 적용 ✅

_완료일: 2026-07-23_

## 적용 범위

- 공통 레이아웃: 사업자 상세 화면은 모바일 플랫폼 전체 폭을 사용하고 콘텐츠 좌우 여백은 24dp로 통일. 320px 소형 화면에서도 페이지별 고정 폭·오버플로 없이 동일 규칙 적용.
- 트레이너 회원 관리: 회원 검색, 검색 결과 없음·초기화, 회원 상세, 피드백 입력 검증·전송·영속 저장 구현.
- 센터 회원 관리: 담당 트레이너 선택·배정, 목록 즉시 반영, 앱 재시작 후 배정 상태 복원 구현.
- 루틴 관리: 기존 Repository 루틴과 마켓 루틴 통합 표시, 새 루틴 이름·설명 검증, 무료 플랜 한도, 저장·복원 구현.
- 상담 수신함: 전체·미답변 필터, 미답변 배지, 답변 검증·전송, 답변 상태·내용 영속 저장 구현.
- 소속 트레이너: 이름 검색, 우수 성과·피드백 필요 필터, 빈 결과 복구, 상세 성과 진입, 초대 링크 복사 구현.
- 정산·매출: 코칭 유형·트레이너·회원 검색, 정산 예정·검토 필요 필터, 빈 결과 복구, 기존 환불·트레이너별·수수료 상세 진입 유지.
- 기존 회원·정산·통계·설정·상담 리타겟 상세 화면의 외부 여백을 공통 토큰으로 정리.

## Phase D-6 선행 적용

- 운영자 회원 관리: 닉네임·이메일 검색, 정상·제한·프리미엄 필터, 빈 결과, 회원 상세, 제재 사유 검증, 제재·해제 확인 절차와 감사 상태 영속 구현.
- 운영자 인증 심사: 승인 확인, 배지 발급, 반려 사유 검증, 승인·반려 상태와 최근 처리 이력 영속 구현.
- 모바일 폰트 기준 심사 바로가기 카드 높이를 보정해 320px 오버플로 제거.

## 검증

- `business_detail_test.dart`: 검색·필터·빈 상태 복구, 피드백·배정·루틴·상담·제재·심사 저장 복원과 7개 상세 화면의 320px 전체 폭·24dp 여백 검증.
- `flutter analyze`: No issues.
- `flutter test`: 전체 테스트 통과.
- `flutter build web`: 성공, Wasm dry run 성공.
- 실제 브라우저 320×568에서 전체 폭 모바일 레이아웃과 콘솔 오류 없음 확인.

## 다음 재개 지점

**Phase D-6 운영자 상세 화면군**의 남은 시스템·콘텐츠 관리 화면부터 시작한다.

1. 신고·제재·미성년 위험·루틴 심사의 확인 절차와 처리 결과 저장.
2. 랭킹·OCR·플랜·금칙어·시스템 로그의 검색·필터·검증·감사 로그 상태 보강.
3. 운영자 전체 화면 320/390px·접근성 글자 크기 회귀 테스트.
4. Phase E React UX 패리티 체크리스트로 이동.
