...
- **달력에서 색을 쓰는 곳은 한 칸당 하나다.** 오늘=브랜드가 채운 원, 막대=그날 **부위 색**
  (둘 이상이면 그라데이션, 길이=완료율). 칸의 글자는 종목이 아니라 부위("가슴 · 등")다 —
  종목명은 잘려서 읽히지 않았다. 부위 색 배정은 `_muscleColor`(홈)와 운동 목록의
  `_muscleCategories`가 같아야 한다. 예전엔 완료율 틴트까지 얹어 한 칸에 색이 셋이었다.
