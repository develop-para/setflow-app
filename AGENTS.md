...
- **미는 조작에는 손잡이가 없다.** 그래서 아직 밀어본 적 없는 사용자(`hasSwipedSet`)에게는
  차례인 행이 **30px 밀린 채 쉬고**, 드러난 트랙 위에서 화살표 두 개가 내비게이션 경로처럼
  순차로 깜빡인다(2.4초 뒤 모션은 멈추지만 자세와 화살표는 남는다 — 가르치는 것은 움직임이
  아니라 상태다). 한 번 밀면 영구히 꺼지고, 애니메이션을 끈 사용자에게는 모션 없이 자세만
  보인다. 애니메이션은 유한해야 한다 — 무한 repeat는 pumpAndSettle을 영원히 돌리고, 토스트
  표시 3초보다 길면 settle이 토스트를 통째로 넘겨버린다.
