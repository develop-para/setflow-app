...
- **휴식은 작은 이동식 창이다.** `FloatingRestTimer`는 약 240px 폭으로 시계·남은 세트·
  `+30초`·끝내기를 보여준다. 드래그로 옮기되 세이프에리어 안에 머문다. 바깥 터치는
  작은 시간 표시로 접고, 그 표시를 누르면 다시 열린다. 접어도 타이머는 계속 간다.
  새 세트의 휴식이 시작될 때만 다시 펼친다 — 매초 갱신이나 시간 연장은 창을 열지 않는다.
  회귀는 `test/rest_focus_test.dart`에서 이동·바깥 터치·시간 유지·작은 화면을 검증한다.
