...
잘못 완료되면 휴식 타이머까지 켜진다. 밀 때 깔리는 트랙은 **드러난 만큼만 자라는 둥근 패널**이고
(폭 = progress), 색은 드래그에 따라 짙어진다. 전체를 채우는 워시로 두면 행의 둥근 모서리 옆에
트랙의 각진 색 조각이 노출된다(실기기 보고). `Dismissible`과 행을 **하나의 `ClipRRect`로
...
- **휴식은 화면을 덮지 않는다.** 헤더 아래 잉크색 바(`GlobalRestTimerOverlay`) 하나가
  휴식의 유일한 얼굴이다 — 시계, **지금 어디쯤인지**(남은 세트·다음 종목), `+30초`, 끝내기가
  전부 한 줄에 항상 보인다. 화면을 덮는 판(`RestFocusOverlay`)은 2026-09-03에 뺐다: 테스터들이
  불편해했고, 쉬는 동안 기록을 보고 다음 세트를 미리 보는 건 딴짓이 아니다. 휴식이 끝난 줄
  모르는 문제는 소리·진동·알림이 답한다. 덮는 판·탭해야 펼쳐지는 버튼으로 되돌리지 말 것.
  "어디쯤인지"는 `toggleSet`이 타이머를 켜기 **전에** 계산해 `startRestTimer(focus:)`로 넘긴다 —
  알림 둘째 줄이 시작 인텐트에 실려서, 뒤늦게 채우면 한 세트 전 얘기가 올라간다. 세트가 아닌
  휴식(함께 방 공유 휴식)은 focus 없이 시작해 지난 세트 얘기를 남기지 않는다.
...
- **브랜드를 글자로 써야 하면 `brandDeep`, 배경 틴트로 써야 하면 `brandSoft`다.**
  ...
  테마 따라 뒤집히는 면 위에서는 상수 대신 **`context.setflowColors.brandDeep/brandSoft`**
  (다크는 들어올린 라임/깊은 올리브). 글자로 된 행동(TextButton)·입력 포커스·선택된 줄은
  테마가 이미 읽는 브랜드(`colorScheme.secondary`)로 칠한다 — 개별 화면에서 잉크로 되돌리지 말 것.
